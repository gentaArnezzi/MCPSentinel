...
- [`authorized_positive_metadata_v3`](authorized_positive_metadata_v3/README.md) is a 16-case Apache-2.0 licensed, source-attributed positive calibration corpus. It contains only metadata-visible, intentionally malicious Cisco evaluation fixtures and preserves their first-party scenario labels.
- [`server_instructions_v4`](server_instructions_v4/README.md) is a 28-case curated regression corpus for dedicated `MCP-S001` review, with balanced benign/malicious English and non-English segments plus obfuscated and ambiguous controls.

The v1 `manifest.json` expands deterministically to 200 descriptors: 35 hand-curated synthetic cases and 165 synthetic-template cases. A template matrix declares every dimension and its `expected_count`; the benchmark validates that count before measuring anything. Generated IDs, input order, and provenance are stable, and each report includes a SHA-256 digest of the source manifest and scanner version, so another reviewer can reproduce the exact corpus from the repository revision.
...
Read [LABELING.md](LABELING.md) before extending any corpus. No bundled corpus is independent evidence of public-server recall, general real-world accuracy, or a competitor comparison.
